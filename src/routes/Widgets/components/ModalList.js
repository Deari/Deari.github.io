                ...
                newList.map((item, index) => (
                  <div className='popup-list-container' key={index}>
                    ...
                    <div className='popup-info-content w320'>
                      <p className='popup-info-name'> {item.appName}<i className={item.appKind === 0 ? 'icon-rnpng' : item.appKind === 1 ? 'icon-hpng' : 'icon-apkpng'} /></p>
                      <p className='popup-info-introduce'> {item.appDesc}</p>
                      <Link className='popup-info-link' to={typeUrl}>在{typeTxt}市场中查看<i className='iconfont icon-categoryindi' /></Link>
                    </div>
                    ...
                    <div className='popup-info-status w140'>
                      {
                          item.versions[0].publishStatus ?
                            item.versions.slice(0, 1).map((v, k) => {
                              return (
                                <div key={k}>
                                  <span className='info-status-info1'><i className={this.getStatus(item, v).activeColor == 'red' ? 'color-red' : this.getStatus(item, v).activeColor == 'green' ? 'color-green' : ''} />{v.codeVersion}</span>
