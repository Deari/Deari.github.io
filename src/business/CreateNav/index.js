...
const Nav = ({ type }) => {
  return <div className={s.createNew}>
    ...
    <div className={s.links}>
      <Link to={`/${type}/create/mini_program`} className={cx(s.item, s.rn)} >
        <i className='iconfont icon-react-native' />
        <span>FAP小程序</span>
      </Link>
      <Link to={`/${type}/create/h5`} className={cx(s.item, s.h5)} >
        <i className='iconfont icon-html5' />
        <span>HTML5</span>
      </Link>
      { type === 'apps' && <Link to={`/${type}/create/apk`} className={cx(s.item, s.apk)} >
