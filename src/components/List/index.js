          ...
          data.map( (item, index) => (
            <div key={index} className="list-container">
              ...
              <div className="info-btn w114">
                <Link className={name!=='hardware'&&!item.codeId || item.reviewStatus==3 || item.hardwareStatus==2 ? 'active':'' } to={'/' + name + 's/edit/' + item[name+'Id']}>
                  <button disabled={name !== 'hardware' && !item.codeId || item.reviewStatus == 3 || item.hardwareStatus == 2 ? false : true}>编辑</button>
                </Link>
                <Link className={item.codeId || item.hardwareStatus==0  ? 'active': ''} to={'/' + name + 's/edit/' + item[name+'Id']} >
