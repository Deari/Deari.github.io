...
class AppsDetail extends React.Component {
  render() {
    ...
    const len = data && data.versions && data.versions.length  
        console.log(this.props.latestVersion)
    return (
...
export const BasicInfo = (props) => {
  ...
  return <div className="detail-container">
    <div className="detail-download">
      <img className="appImg" src={  latestVersion.publishStatus?data.appLogo:data.changes&&data.changes.appLogo } alt="LOGO"/>
    ...
    <div className="detail-info">
      <dl className="detail-tittle">
        <dt>{ latestVersion.publishStatus ? data.appName : data.changes&&data.changes.appName}</dt>
        <dd><i className="user-img"></i><span>{data.developerName}</span></dd>
      </dl>
      <h3 className="app-title">内容提要</h3>
      <p className="app-text">{ latestVersion.publishStatus?data.appDesc:data.changes&&data.changes.appDesc}</p>
